...
client_info client[4];
void cast_msg(string msg){
    cout<<"size : "<<msg.length()<<endl<<msg<<endl;
    for(int i = 0;i<=connect_count;i++)
        send(client[i].connfd,msg.c_str() , msg.length(), 0);
    }
...
void thread_task(int connfd,int p) { // the task which every thread should execute/
    int n, cnt=0;
    ...
    while(1) { // receive user message and return response using loop.
        ...
        cout<<n<<" : "<<buff<<endl;
        buff[n] = '\0';
        if(cnt++ == 0) client[connect_count-1].name = buff;
        ...
        if(command[0] == "create") { // create file
            ...
            client[p].name = command[1];
            string payload = "000102030506090807";
            string content = buff;
            string msg ="connect count:"+to_string(connect_count)+"payload:"+ payload +"client content:"+content;
            
            cast_msg(msg); 
        }else if(command[0] == "player"){
            cout<<"current connect : "<< connect_count<<endl;
            string payload;
            for(int i =0;i<connect_count;i++)
                payload +=  client[i].name+" ";
            string msg ="player "+to_string(connect_count)+" "+ payload ;
            cout<<"msg : "<< msg<<endl;
            cast_msg(msg); 
            ...
            if(!isFirst){
                string msg = shuffle_string;
                cast_msg(msg);
            }
        }else if(command[0] =="whoFirst"){
            if(!isFirst){
                cout<<"("<<to_string(first_player)<<")"<<client[first_player].name<<"\n";
                string msg ="whoFirst "+ to_string(first_player);
                cast_msg(msg); 
                isFirst = true;               
            }
        ...
        //send(connfd+1, "ack!!\n", sizeof("ack!!\n"), 0);
        //send(connfd, "t:000102030506090807\n", sizeof("t:000102030506090807\n"), 0);
...
//-----shuffle------------------------
    ...
    for(int i:a){
        shuffle_string += to_string(i) +" ";
    }
    cout<<shuffle_string<<endl;

    srand( time(NULL) );
    first_player = 0 + ( std::rand() % ( 3 - 0 + 1 ) );
//--------------------------------------
    ...
    thread t[4]; // the maximum connection number of user
    int cnt = 0;
    while(1) { // wait for user connection
        unsigned int clientaddrlen = sizeof(clientaddr);
        int connfd;
        if((connfd = accept(listenfd, (struct sockaddr*) &clientaddr, &clientaddrlen)) != -1) {
            cout << "thread enter!! : " <<connfd<<endl;
            connect_count++;
            client[cnt].connfd = connfd;
            client[cnt].seq_num = cnt;
            t[cnt] = thread(thread_task, connfd,cnt);
            string msg = "Welcome! "+to_string(cnt);
            send(client[cnt].connfd,msg.c_str() , msg.length(), 0);
